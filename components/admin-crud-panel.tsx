...
export function AdminCrudPanel({ permissions }: { permissions: string[] }) {
  ...
  return <section className="crud-workspace" id="content-desk">
    <div className="crud-heading"><div><p className="eyebrow">Content desk</p><h2>Manage records</h2></div><select value={type} onChange={(event) => { setType(event.target.value); setEditing(null); setValues({}); setEditingStatus(null); setMediaFile(null); setAltText(""); }} aria-label="Content type">{permittedTypes.map((value) => <option key={value} value={value}>{value}</option>)}</select></div>
    {(canManage || (canEdit && editing !== null)) && <form className="crud-form" onSubmit={save}><h3>{editing === null ? "Create" : "Edit"} {type}</h3>{fieldMap[type].map((field) => <label key={field}>{field}{field === "is_active" ? <input type="checkbox" checked={values[field] !== "0" && values[field] !== "false"} onChange={(event) => setValues({ ...values, [field]: event.target.checked ? "1" : "0" })} /> : field === "side" ? <select value={values[field] ?? "left"} onChange={(event) => setValues({ ...values, [field]: event.target.value })}><option value="left">Left</option><option value="right">Right</option></select> : <input required={!['description', 'summary', 'body', 'eyebrow', 'cta_label', 'cta_url', 'image_url', 'location', 'sort_order'].includes(field)} value={values[field] ?? ""} onChange={(event) => setValues({ ...values, [field]: event.target.value })} />}</label>)}{editing !== null && editingStatus && workflowTypes.has(type) && <label>Status<select value={editingStatus} onChange={(event) => setEditingStatus(event.target.value)}>{statuses.map((status) => <option key={status} value={status}>{status}</option>)}</select></label>}{canUploadMedia && <><label>Image<input type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={(event) => setMediaFile(event.target.files?.[0] ?? null)} /></label><label>Image alt text<input value={altText} onChange={(event) => setAltText(event.target.value)} /></label></>}<div className="crud-form-actions"><button type="submit">{editing === null ? "Create record" : "Save changes"} ↗</button>{editing !== null && <button type="button" onClick={() => { setEditing(null); setValues({}); setEditingStatus(null); setMediaFile(null); setAltText(""); }}>Cancel</button>}</div></form>}
